package com.example.bro.videotrimer.RangeSeekbar;
...
import com.example.bro.videotrimer.R;
...
public class CrystalRangeSeekbar extends View {
    ...
    public CrystalRangeSeekbar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try{
            ...
            barPadding             = getBarPadding(array);
        }
        finally {
            array.recycle();
        }
    ...
    public int pressedThumb(){
        Thumb t = getPressedThumb();
        if (t == null)
            return 0;
        if (Thumb.MIN.equals(t))
...
//        barPadding = thumbWidth * 0.5f;
    ...
    protected float getDiameter(final TypedArray typedArray){
        return typedArray.getDimensionPixelSize(R.styleable.CrystalRangeSeekbar_thumb_diameter, 10);
    }
    protected float getDiameterHeight(final TypedArray typedArray){
        return typedArray.getDimensionPixelSize(R.styleable.CrystalRangeSeekbar_thumb_diameter_height, 10);
    }
    protected float getBarPadding(final TypedArray typedArray){
        return typedArray.getDimensionPixelSize(R.styleable.CrystalRangeSeekbar_bar_padding, 0);
    }
...
//        canvas.drawRoundRect(rect, cornerRadius, cornerRadius, paint);
    ...
    protected void setupLeftThumb(final Canvas canvas, final Paint paint, final RectF rect){
        ...
        rectLeftThumb.right  = normalizedToScreen(normalizedMinValue) + barPadding;
        rectLeftThumb.left   = rectLeftThumb.right - getThumbWidth();
...
//        canvas.drawOval(rect, paint);
    ...
    protected void setupRightThumb(final Canvas canvas, final Paint paint, final RectF rect){
        ...
        rectRightThumb.right = rectRightThumb.left+getThumbWidth();
...
//        canvas.drawOval(rect, paint);
    ...
    protected void trackTouchEvent(MotionEvent event){
        ...
        try{
            ...
            if (Thumb.MIN.equals(pressedThumb)) {
                setNormalizedMinValue(screenToNormalized(x+delta));
            } else if (Thumb.MAX.equals(pressedThumb)) {
                setNormalizedMaxValue(screenToNormalized(x-delta));
            }
        }
    ...
    private Thumb evalPressedThumb(float touchX){
        ...
        boolean minThumbPressed = isInThumbRange(touchX, normalizedMinValue, Thumb.MIN);
        boolean maxThumbPressed = isInThumbRange(touchX, normalizedMaxValue, Thumb.MAX);
    ...
    private boolean isInThumbRange(float touchX, double normalizedThumbValue, Thumb pressedThumb){
        float thumbPos = normalizedToScreen(normalizedThumbValue);
        float left;
        float right;
        if (pressedThumb == Thumb.MIN) {
            left = thumbPos - getThumbWidth();
            right = thumbPos + (0.25f * getThumbWidth());
        }
        else {
            left = thumbPos - (0.25f * getThumbWidth())  ;
            right = thumbPos + getThumbWidth();
        }
    ...
    public synchronized boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (pressedThumb == Thumb.MIN)
                    delta = normalizedToScreen(normalizedMinValue) -event.getX()+barPadding;
                else
                    delta = event.getX() - normalizedToScreen(normalizedMaxValue) - barPadding;
